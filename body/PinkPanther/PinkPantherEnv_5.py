...
def get_action(steps):
	params = np.array([0.13, 0.0, 0.17, 0.2, 0.3]) # 0.5m/s consistent Real World
	params = np.array([0.15, 0.0, 0.15, 0.15, 0.5])
	#params = np.array([0.15, 0.0, 0.2, 0.15, 0.2]) # Smooth Criminal, Jul 31, 19:00
	return act(steps, *params)

def act(t, p0, p1, p2, p3, p4):
	...
	# back positive
	b_pos = p2 * np.sin(t * p4) + p3
	# back negative
	b_neg = -p2 * np.sin(t * p4) + p3
