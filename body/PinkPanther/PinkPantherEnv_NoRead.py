...
class PinkPantherEnv(gym.Env):
	...
	def act(self, action):
		...
		for i in range(len(action)):
			...
			if i>5:
				p.setJointMotorControl2(self.robotid, i, controlMode=self.mode, targetPosition=action[i], force=self.params['maxForce']/1.8, maxVelocity=self.params['maxVel']/1.)
			else:
