...
def weight_para(para, indx):
	# amplitude for front motors
	if indx in [0, 2]:
		para *= 0.4
	# amplitude for back motors
	if indx in [4, 6]:
		para *= 0.4
	# offset for front motors
	if indx in [1, 3, 5, 7]:
		para *= 0.1
	else:
		pass
...
def act(t, para):
	# front shoulder (+p1)
	f_s = para[0] * np.sin(t * para[8])
	# front elbow (+p3)
	f_e = para[2] * np.sin(t * para[8])
	# back shoulder (+p5)
	b_s = para[4] * np.sin(t * para[8])
	# back elbow (+p7)
	b_e = para[6] * np.sin(t * para[8])
	desired_p = np.array([0, f_s+para[1], f_e+para[3], 0, -f_s+para[1], -f_e+para[3], 0, -b_s+para[5], -b_e+para[7], 0, b_s+para[5], b_e+para[7]])
	return desired_p
...
if __name__ == '__main__':
	from body.PinkPanther.PinkPantherEnv_9 import PinkPantherEnv
	...
	for i in range(epochs):
		...
		path = os.path.join('body/PinkPanther/params/train0', 'best_epoch{}'.format(i+1))
		np.save(path, best[1])
