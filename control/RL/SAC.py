...
class SAC(object):
    def __init__(self, env, actor_critic=MLPActorCritic, ac_kwargs=dict(), seed=0,
        ...
        logger_kwargs=dict(), save_freq=1, log_dir='./'):
        """
    ...
    def update(self, batch_size=None, n_updates=None):
        if self.steps >= self.update_after and self.steps % self.update_every == 0:
            for j in range(self.update_every):
                ...
                loss_pi, loss_q = self.__update__(data=batch)
    ...
    def learn(self, total_timesteps, test_env=None, log_interval=1000):
        ...
        for t in range(total_timesteps):
            if t % log_interval == 0:
                if test_env is None:
                    test = self.test(self.env)
                else:
                    test = self.test(test_env)
                test_scores.append([t, test[:,1]])
                print('Timesteps: '+str(t))
                print('Return: '+str(np.mean(test[:,1])))
                print('Length: '+str(np.mean(test[:,0])))
                print('Time: '+str(round(time.time()-start_time, 3)))
                # print('Avg Length: '+str(np.mean(tmp_scores[-100:,0])))
                print('Avg Return: '+str(np.mean(rews)))
                if last_loss is not None:
                    print('Q Loss: '+str(last_loss[0]))
                    print('Pi Loss: '+str(last_loss[1]))
                print('')
                np.save(self.log_dir+'/test_log.npy', test_scores)

            self.steps = t

            a = self.act(np.expand_dims(o, 0))
            ...
            self.replay.add(o, a, o2, r, False)
            ...
            if d or (ep_len == max_ep_len):
                train_scores.append([t, ep_ret])
                rews.append(ep_ret)
                if self.log_dir is not None:
                    if not os.path.exists(self.log_dir):
                        os.makedirs(self.log_dir)
                    np.save(self.log_dir+'/train_log.npy', train_scores)
                ep_n += 1
                # if ep_n % log_interval == 0:
    ...
    def test(self, env, n_episodes=5):
        ...
        for ep in range(n_episodes):
            ...
            while not done:
                obs = torch.FloatTensor(obs.reshape(1, -1)).to(device)
                action = self.get_action(obs).cpu().detach().numpy().flatten()
                # a = self.act(np.expand_dims(obs, 0))
                # a = a.cpu().detach().numpy().flatten()
...
if __name__ == '__main__':
    ...
    sac.learn(total_timesteps=1000000)
